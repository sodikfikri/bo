 ...
 *
 */
class Request_register extends CI_Controller
{
  var $listMenu = "";
  ...
  function __construct()
  {
    ...
    $this->system_model->checkSession(26);
  ...
  function index(){
    ...
    $sql = $this->institution_model->getAllSession();
    $no = 0;
    foreach ($sql as $row) {
	  $no++;
	  ...
	  $chkbox = '';
	  if($row->cabang_area_id!=0){
		  $chkbox = '<input type="checkbox" name="order-id[]" value="'.$encId.'">';
	  }
	  
	  $detailEmployee  = '<span class="text-blue" style="cursor:pointer;font-weight:600" onclick="detail(\''.$encId.'\')">total : '.$row->totalEmployee.'<i class="fa fa-play-circle fa-lg" style="margin-left:8px"></i></span></span>';
      $option  = $btnEdit;
      $this->table->add_row(
	    $chkbox,
    ...
    $parentViewData = [
      "title"   => $this->gtrans->line("Request Register"),  // title page
      "content" => "master/request_register",  // content view
      "viewData"=> $data,
  ...
  function approveAllBranch(){
    ...
    foreach ($arrOrder as $rowID) {
		...
		$this->checkout_cart_model->activeEmpOrder($orderid,$data_updateOrder);
	}
    echo 'success update order license and employee license is active';
  }
  ...
  function deleteCabang($encId){
    $this->load->library("encryption_org");
    $id  = $this->encryption_org->decode($encId);
    $res = $this->institution_model->delete($id);
    if($res){
      setActivity("master institution","delete");
      $this->session->set_userdata("ses_msg",["type"=>"success","header"=>"Success","msg"=> $this->gtrans->line("Institution has been deleted successfully")."!"]);
      $this->gtrans->saveNewWords();
      redirect("master-institution");
    }
  }
  ...
  function checkNameExists(){
    ...
    $appid    = $this->session->userdata("ses_appid");
    $nameExists = $this->institution_model->isNameExists($cabangName,$cabangid,$appid);
