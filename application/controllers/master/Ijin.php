...
class Ijin extends CI_Controller
{
  ...
  function index(){
    ...
    foreach ($dataIjin as $row) {
      ...
      $btnEdit = '<i style="cursor:pointer" class="fa fa-edit fa-lg color-primary" onclick="edit(\''.$encId.'\',\''.$encName.'\',\''.$encDescription.'\')"></i>';
  ...
  function deleteIjin($encId){
    ...
    if($res){
      setActivity("master ijin","delete");
