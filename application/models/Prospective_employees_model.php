 ...
 *
 */
class Prospective_employees_model extends CI_Model
{
  ...
  function insertOrder($dataInsert,$appid=""){
    ...
    if(!empty($appid)){
      if(!empty($this->session->userdata("ses_userid"))){
        $userID = $this->session->userdata("ses_userid");
        $dataInsert["user_add"]    = $userID;
      }
      
      $dataInsert["appid"]        = $appid;
      $dataInsert["status"]       = 'pending';
      $dataInsert["datecartopen"] = $this->now;

      $res = $this->db->insert($this->tableOrder,$dataInsert);
      if($res){
        return $this->db->insert_id();
      }else{
        return false;
  ...
  function updateOrder($insertResult,$appid=""){
    if($appid==""){
      $appid = $this->session->userdata("ses_appid");
    }
    ...
    if(!empty($appid)){
	  ...
	  $this->db->where("user_add",$userID);
	  $this->db->where("status","pending");
	  $this->db->where("order_id !=",$insertResult);
  ...
  function getAll($appid=null,$dataUrl){
    if ($appid==null) {
      $appid = $this->session->userdata("ses_appid");
    }
    ...
    if(!empty($appid)){
      ...
      (
        select
        ifnull(count(order.order_id),0)
        from
        order
        where
		order.appid = tbemployee.appid AND
        order.order_id = tbemployee.parent_order_id AND
        order.status = 'active'
      ...
      $this->db->where("paymentActive","0");
      $this->db->where("tbemployee.is_del !=","1");
      $this->db->where("tbemployeeareacabang.employee_cabang_id",$dataUrl);
      $this->db->where("tbemployee.employee_jenis_modif !=","delete");
      ...
      $this->db->where("resign_confirmed","no");
      $this->db->order_by("tbemployee.","ASC");
