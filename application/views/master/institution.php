...
<section class="content-header">
  <h1>
    <?= $this->gtrans->line("Master Institution") ?>
  </h1>
</section>
    ...
    <div class="box box-inact">
      ...
      <div class="box-body">
        <div class="row">
          <div class="col-md-12">
            <?= !empty($notif) ? $notif : "" ?>
            <button type="button" class="btn btn-primary" data-toggle="modal" onclick="addNew()"><i class="fa fa-pencil"></i> <?= $this->gtrans->line("New Institution") ?></button><br><br>
...
<script type="text/javascript">
        ...
        if(stat%2==0){
          ...
          if(existingTimezone!=$("#timezone").val()){
            Swal.fire({
              ...
              text: "Is the data entered correct?",
              icon: 'warning',
  ...
  function addNew(){
    ...
    $("#frm-text").html('<i class="fa fa-building" style="font-size:30px"></i> <?= $this->gtrans->line("Institution Information") ?>');
    $("#frm-desc").html('<?= $this->gtrans->line("Please provide correct information about the Institute") ?>');
    $("#id").val("");
    $("#area").val("");
    $("#institutioncode").val("");
    $("#institutionname").val("");
  ...
  function editMethod(id,method,mode){
    ...
    $("#frm-text-method").html('<?= $this->gtrans->line("Edit Institution Setting") ?>');
    $("#idmethod").val(id);
  ...
  function redirectPage(){
	  var kodeID = document.getElementById('idBranch').value;
	  window.open(url+'master-prospective-employees/'+kodeID);
  }
  function detail(encID){
    $("#loader").fadeIn(1);
	$("#show-detail-employee").html('<?= $this->gtrans->line("Began to register employees in this institution") ?>');
	$("#show-text-desc").html('<?= $this->gtrans->line("Add Employee to get license") ?>');
	$("#txtBtnAdd").html('<i class="fa fa-floppy-o"></i> <?= $this->gtrans->line("Register Employees") ?>');
	$("#idBranch").val(encID);
	$("#txtBtnImport").html('<i class="fa fa-file-excel-o"></i> <?= $this->gtrans->line("Import Excel") ?>');
  ...
  function delInstitution(idcabang,totalDevice,totalEmployee){
    if(totalEmployee== 0){
      Swal.fire({
        ...
        if (result.value) {
          window.open(url + 'delete-institution/' + idcabang,'_self');
        }
      });
    }else{
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: 'Institution is used by several device or employee!'
