...
class Cabang_model extends CI_Model
{
  ...
  function getAll($appid=""){
    ...
    if(!empty($appid)){
      ...
      (
        select
        count(tbdevice.device_id)
        from
        tbdevice
        where tbdevice.device_cabang_id = tbcabang.cabang_id
        and tbdevice.is_del !='0'
      ) as totalDevice
      ...
      $this->db->join("tbarea","tbarea.area_id = tbcabang.cabang_area_id");
  ...
  */
  ...
  function getActiveBranchIdentification($appid){
    $this->db->select("cabang_id");
  ...
  function getActiveBranch($appid){
    $this->db->select("cabang_id");
