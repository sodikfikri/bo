          ...
          <li class="dropdown user user-menu">
            ...
            <ul class="dropdown-menu" style="background-color:#039be5">
              ...
              <li class="user-header">
				...
				<?php if($this->session->userdata("ses_userStat")=='1'){ ?>
				  <button type="button" class="btn btn-secondary" name="" data-toggle="modal" data-target="#modal-registration-link"><i class="fa fa-link"></i> <?= $this->gtrans->line("Registration Link") ?></button>
				<?php } ?>
              ...
              <li class="user-body">
                <div class="row">
				  ...
				  <?php } else { ?>
					<a href="<?= base_url('logout-intrax'); ?>" style="color:#fff !important;"><i class="fa fa-sign-out"></i> <?= $this->gtrans->line("Sign out"); ?></a>
				  <?php } ?>
...
<div class="modal fade" id="modal-registration-link">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
      ...
      <div class="modal-body">
		  ...
		  <div class="row">
			...
			<div class="col-md-4">
				<button class="btn btn-primary" type="submit" name="share" value="submit" onclick="shareLinkRegister()"><i class="fa  fa-share"></i> <?= $this->gtrans->line('Share') ?></button>
			</div>
		  </div>
      </div>
    <div class="modal-footer">
		<span class="pull-left"><img src="<?= (!empty($this->session->userdata("ses_userImage"))) ? base_url("sys_upload/userpic/".$this->session->userdata("ses_userImage")) : base_url("asset/images/admin-icon.png") ?>" class="user-image" alt="User Image" width="30px"> <?= ucwords($this->session->userdata("ses_companyname")) ?></span>
    </div>
  </div>
  </div>
</div>
...
function shareLinkRegister(){
  $("#msg-link").html('Processing...');
  var email_intrax = $("#email_intrax").val();
  var link_intrax = $("#link_intrax").val();
  if (email_intrax!='') {
	  $.ajax({
      method : "POST",
      url    : url + "send-link-registration",
      data   : {email_intrax:email_intrax,link_intrax:link_intrax},
      success: function(res){
        if(res=="success"){
          $("#msg-link").html('Link Registration was sent');
        }else{
          $("#msg-link").html('Processing Failed!');
        }
      }
    })
  } else {
