...
class History_log extends CI_Controller
{
  ...
  function index()
  {
    ...
    $parentViewData = [
      ...
      "content" => "report/history_log",  // content view
      "viewData"=> $data,
  ...
  function loadDataFromFinalTable()
  {
	...
	$queryCheckAll = "select count(id) as total from tbfinal_checkinout
									 where appid='$appid' and (DATE(datetime) between '$datestart' and '$dateend')";
	$sqlCekAll = $conn->prepare($queryCheckAll,array(PDO::MYSQL_ATTR_FOUND_ROWS => true));
	$sqlCekAll->execute();
	$dataCountAll = $sqlCekAll->fetch();
		
	$allRecord  = $dataCountAll["total"];
	...
	$filterTemperature = "";
	if($sName!=""){
	  $filterText = " and (account_no like '%$sName%' or name like '%$sName%') ";
	}
	if($sArea!=""){
	  $filterArea = " and area_id='$sArea' ";
	}
	if($sCabang!=""){
	  $filterCabang = " and cabang_id='$sCabang' ";
	}
	if($temperature !=""){
	  $filterTemperature = " and temperature ".$temperature." ";
	}
	$mainQuery = "select * from tbfinal_checkinout where appid='$appid' and (DATE(datetime) between '$datestart' and '$dateend') ".$filterText.$filterArea.$filterCabang.$filterTemperature." order by `datetime` DESC";
	$sqlPaginate = $conn->prepare($mainQuery." limit ".$start.",".$length);

	$sqlPaginate->execute();
	$dataMain = $sqlPaginate->fetchAll();

	$sqlCekFiltered  = $conn->prepare($queryCheckAll.$filterText.$filterArea.$filterCabang,array(PDO::MYSQL_ATTR_FOUND_ROWS => true));
	$sqlCekFiltered->execute();
	$dataCekFiltered = $sqlCekFiltered->fetch();
	$output['recordsFiltered'] = $dataCekFiltered["total"];
	...
	foreach ($dataMain as $row) {
		...
		$row["name"],
		$row["datetime"],
		'<div style="text-align:center">'.$row["absen_code"].'</div>',
		'<div style="text-align:center">'.$row["verify_code"].'</div>',
		'<div style="text-align:center">'.$row["temperature"].'</div>',
		'<div style="text-align:center">'.$row["use_masker"].'</div>',
		);
	}
  ...
  function reportPrintFromFinalTable()
  {
    load_model(["area_model","cabang_model"]);
    $this->load->library("dbconnection");
    $conn = $this->dbconnection->connect();
    ...
    $cabang = $_GET["cabang"];
    $term   = $_GET['term'];
    $temperature   = !empty($_GET['temperature']) ? base64_decode($_GET['temperature']) : "";
    ...
    $this->table->set_heading(
      ...
      $$filterTemperature  = "";
      if($term!=""){
        $filterText = " and (account_no like '%$term%' or name like '%$term%') ";
      }

      if($area!=""){
        $filterArea = " and area_id='$area' ";
      }

      if($cabang!=""){
        $filterCabang = " and cabang_id='$cabang' ";
      }
      ...
      $mainQuery = "select * from tbfinal_checkinout
                                   where appid='$appid' and (DATE(datetime) between '$dateFrom' and '$dateTo') ".$filterText.$filterArea.$filterCabang.$filterTemperature." order by `datetime` DESC";
    // echo $mainQuery; exit;
      $mainSql = $conn->prepare($mainQuery);
      $mainSql->execute();
      $mainData = $mainSql->fetchAll();
      $no = 0;
      
      foreach ($mainData as $row) {
        $no++;
        $arrDateTime = explode(" ",$row["datetime"]);
        // $labelVerifType = ' ('.$this->getVerificationType($row["verify_code"]).')';

        $this->table->add_row(
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["sn"].'</p>'
          ],
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["account_no"].'</p>'
          ],
          '<p class="table-content">'.$row["name"].'</p>',
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$arrDateTime[0].'</p>'
          ],
          ...
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["absen_code"].'</p>'
          ],
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["verify_code"].'</p>'
          ],
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["temperature"].'</p>'
          ...
          [
            "style"=> "text-align:center",
            "data" => '<p class="table-content">'.$row["use_masker"].'</p>'
          ]
        );
      }
    //}
    ...
    $data["title"] = "Report History Log ".$dateFrom." - ".$dateTo;
    $data["subtitle"] = "Area : ".$areaName.", Branch : ".$cabangName.", Temperature : ".$temperature.", Name or No Account Have String : ".$term;
    ...
    */
    ...
    $data["tabel"] = (count($mainData)>0) ? $this->table->generate() : '<p style="text-align:center">No Data Found!</p>';
    $data["type"]  = "print";
    $this->load->view("report/print_layout",$data);
  }
