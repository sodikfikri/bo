 ...
 * 
 */
class Thermorequest_model extends CI_Model
{
  ...
  function toDeviceTimeZone($zone){
    ...
    if($newData->num_rows()>0){
      foreach ($newData->result() as $row) {
        // add user
        if($SN=="6209212200056"){
          $c1 = "C:AU.".$row->employeeareacabang_id."|".$row->employee_id.":DATA UPDATE USERINFO PIN=".$row->employee_account_no."\tName=".$row->employee_nick_name."\tPri=".$row->employee_level."\tPasswd=".$row->employee_password."\tCard=".$row->employee_card."\tGrp=1\n";
		  $output .= $c1;
        }else{
          $c1 = "C:AU.".$row->employeeareacabang_id."|".$row->employee_id.":DATA UPDATE USER Pin=".$row->employee_account_no."\tName=".$row->employee_nick_name."\tPrivilege=".$row->employee_level."\tPassword=".$row->employee_password."\tCardNo=".$row->employee_card."\tGroup=1\tStartTime=0\tEndTime=0\n".
            "C:".$row->employeeareacabang_id.":DATA UPDATE userauthorize Pin=".$row->employee_account_no."\tAuthorizeTimezoneId=1\tAuthorizeDoorId=1"."\n";
                $output .= $c1;
        }
      ...
      if($updateData->num_rows()>0){
        foreach ($updateData->result() as $rowUpdate) {
          $c2 = "C:UU.".$rowUpdate->employeelocationdevice_id.":DATA UPDATE USER Pin=".$rowUpdate->employee_account_no."\tName=".$rowUpdate->employee_nick_name."\tPrivilege=".$rowUpdate->employee_level."\tPassword=".$rowUpdate->employee_password."\tCardNo=".$rowUpdate->employee_card."\tGroup=1\tStartTime=0\tEndTime=0\n".
            "C:".$rowUpdate->employeelocationdevice_id.":DATA UPDATE userauthorize Pin=".$rowUpdate->employee_account_no."\tAuthorizeTimezoneId=1\tAuthorizeDoorId=1"."\n";
                	$output .= $c2;
        }
      }else{
        // delete user
        ...
        if($deleteData->num_rows()>0){
          foreach ($deleteData->result() as $row) {
            $c3 = "C:DU.".$row->employeeareacabang_id."|".$row->device_id."|".$row->employee_id.":DATA DELETE user Pin=".$row->employee_account_no."\n";
            $output .= $c3;
          }
        }else{
          // insert template
          ...
          if($sqlTemplate->num_rows()>0){
            ...
            foreach ($sqlTemplate->result() as $rowTemplate) {
              if($rowTemplate->employeetemplate_jenis=="fingerprint"){
                ...
                $c4 = "C:AT.".$rowTemplate->employeelocationdevice_id."|".$rowTemplate->employeetemplate_id."|".(!empty($rowTemplate->counter) ? $rowTemplate->counter : "0").":DATA UPDATE templatev10 Pin=".$rowTemplate->employee_account_no."\tFingerID=".$rowTemplate->employeetemplate_index."\tValid=1\tTemplate=".$rowTemplate->employeetemplate_template."\n";
              }else{
            ...
            if($mutationOut->num_rows()>0){
              foreach ($mutationOut->result() as $row) {
                // mutation out
                $c6 = "C:MO.".$row->employeemutation_id."|".$row->employeemutation_c_id.":DATA DELETE user Pin=".$row->employee_account_no."\n";
                $output .= $c6;
              }
            }else{
              ...
              if($mutationIn->num_rows()>0){
                foreach ($mutationIn->result() as $row) {
                  // mutation in
                  $c5 = "C:MI.".$row->employeemutation_id."|".$row->employeemutation_c_id.":DATA UPDATE user Pin=".$row->employee_account_no."\tName=".$row->employee_nick_name."\tPrimary=0\tPassword=".$row->employee_password."\tCardNo=".$row->employee_card."\tGroup=1\n";
                  $output .= $c5;
                }
              }else{
                // update picture
