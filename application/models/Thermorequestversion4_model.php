 ...
 * 
 */
class Thermorequestversion4_model extends CI_Model
{
    ...
    if($newData->num_rows()>0){
      ...
      if($updateData->num_rows()>0){
        foreach ($updateData->result() as $rowUpdate) {
			if($SN=="CL9M212260073"){
				$c2 = "C:UU.".$rowUpdate->employeelocationdevice_id.":DATA UPDATE USERINFO PIN=".$rowUpdate->employee_account_no."\tName=".$rowUpdate->employee_nick_name."\tPasswd=".$rowUpdate->employee_password."\tCard=".$rowUpdate->employee_card."\tPri=".$rowUpdate->employee_level."\n";
                  $output .= $c2;
			}else{
          ...
          if($sqlTemplate->num_rows()>0){
            ...
            foreach ($sqlTemplate->result() as $rowTemplate) {
              if($rowTemplate->employeetemplate_jenis=="fingerprint"){
                ...
                $c4 = "C:AT.".$rowTemplate->employeelocationdevice_id."|".$rowTemplate->employeetemplate_id."|".(!empty($rowTemplate->counter) ? $rowTemplate->counter : "0").":DATA UPDATE BIODATA Pin=".$rowTemplate->employee_account_no."\tNo=".$rowTemplate->employeetemplate_index."\tIndex=".$rowTemplate->employeetemplate_index."\tValid=1\tDuress=0\tType=".$templateTypeId."\tMajorVer=".$fingerMajorver."\tMinorVer=".$fingerMinorver."\tFormat=".$strFormat."\tTmp=".$rowTemplate->employeetemplate_template."\n";
              }else{
				...
				if($SN=="CQIM232460012" || $SN=="CQIM232460009" || $SN=="CQIM232460042" || $SN=="CQIM232460047" || $SN=="CQIM232460049" || $SN=="CQIM232460040"){
					$c4 = "C:AT.".$rowTemplate->employeelocationdevice_id."|".$rowTemplate->employeetemplate_id."|".(!empty($rowTemplate->counter) ? $rowTemplate->counter : "0").":DATA UPDATE BIODATA Pin=".$rowTemplate->employee_account_no."\tNo=0\tIndex=0\tValid=1\tDuress=0\tType=9\tMajorVer=39\tMinorVer=3\tTmp=".$rowTemplate->employeetemplate_template."\n";
				}else{
            ...
            if($MutasiOutEmployeeCount>0){
              ...
              if($mutationIn->num_rows()>0){
                foreach ($mutationIn->result() as $row) {
                  // mutation in
				  $c5 = "C:MI.".$row->employeemutation_id."|".$row->employeemutation_c_id.":DATA UPDATE USERINFO PIN=".$row->employee_account_no."\tName=".$row->employee_nick_name."\tPasswd=".$row->employee_password."\tCard=".$row->employee_card."\tGrp=1\tPri=".$row->employee_level."\n";
